    ...
    implements ActionListener, KeyListener, MouseListener
{
  ...
  public boolean isRowHittingGround()
  {
    for (int i = 0; i < box2d.size(); i++)
    {
      ...
      {
        if (box2d.get(i)[index] != null)
        {
          if (b.getLocation().getY() + Box.height >= movingBallInitialY)
          {
            return true;
          }
        }
  ...
  public void paintTitleScreen(Graphics g)
  {
    ...
    g.drawString("Press 'P' to play.", (int) (Main.screenSize.getWidth() / 9.5),
        (int) (Main.screenSize.getHeight() / 1.75));
    g.setFont((new Font(Font.SANS_SERIF, Font.BOLD,
  ...
  public boolean isRowNull(int index)
  {
    Box[] rowToCheck = box2d.get(index);
    int count = 0;
    for (Box b : rowToCheck)
    {
      if (b == null)
      {
        count++;
